        ...
        <div className="min-h-[calc(100vh-64px)] bg-slate-950 py-16 px-4">
            <div className="max-w-6xl mx-auto">
                <div className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4">Local Job Board</h1>
                    <p className="text-lg text-slate-400 max-w-2xl mx-auto">Find exciting career opportunities from top businesses in your community.</p>
                </div>
                ...
                {jobs.length === 0 ? (
                    <div className="glass rounded-2xl p-16 text-center border border-slate-700/50 max-w-2xl mx-auto">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-3">No jobs posted yet</h2>
                        <p className="text-slate-400 mb-8">Businesses haven't posted any job openings at this time. Check back later!</p>
                        <Link href="/pricing" className="btn-primary px-6 py-3 rounded-xl text-sm font-bold">Post a Job</Link>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {jobs.map(job => (
                            <div key={job.id} className="glass rounded-2xl p-6 border border-white/10 shadow-sm hover:shadow-xl transition-shadow group flex flex-col">
                                <div className="flex items-center justify-between mb-4">
                                    <span className="px-3 py-1 bg-primary-500/20 text-primary-400 border border-primary-500/30 text-xs font-bold rounded-full">{job.employment_type}</span>
                                ...
                                <p className="text-sm font-semibold text-primary-400 mb-4">{job.listing_name}</p>
                                <p className="text-slate-400 text-sm mb-6 flex-1 line-clamp-3">{job.description}</p>
